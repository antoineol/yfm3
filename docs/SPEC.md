...
- NTSC-U/RP field cursor focus must use the decompiled board cursor mapping: read column `0x0E9F57 + turn*0x70` and row `0x0E9F58 + turn*0x70`, then map through `DAT_800907D8` at `0x0907D8 + turn*0x14 + row*5 + column` to get the live duel-table slot. Player field slots are `5..9`; opponent field slots are `20..24`. `DAT_8009B338` is only the displayed card-info target and can remain stale after Circle cancels attack targeting. `0x09B34E` is a cursor-rendering/card-info mode, not a trusted physical field slot.
...
- NTSC-U/RP attack targeting must follow the decompiled field-state branch. During active target selection, `0x0E9F68 + turn*0x70` is a non-zero target-selection object pointer. Use that pointer only as the active-target flag, then read target column/row from `0x0E9F73 + turn*0x70` and `0x0E9F74 + turn*0x70`, mapping through `DAT_800907D8` (`0x0907D8 + turn*0x14 + row*5 + column`) to get the defender duel-table slot. The selected attacker remains the live board cursor-map slot (`5..9`) underneath the target-selection overlay. After Circle cancels attack targeting, the `0x0E9F68 + turn*0x70` pointer is zeroed by the C path, so focus must return to the cursor-map slot. `DAT_8009B338` can remain stale and `0x09B1F8` is not a battle-target mode byte in the observed NTSC-U/RP state. Do not restore cancelled focus from UI history or card-id history.
