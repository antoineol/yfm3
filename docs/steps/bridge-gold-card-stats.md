...
- Card frame colors are extracted from Gold's packed 4-bit EXE color-category table plus the extracted card type. The table is authoritative for special monster frames; non-monster frames are type-based: Magic/Equip green, Ritual blue, Trap pink. The `{F8 0A XX}` card-name prefix is exported separately as `labelColor` because Mod 15 uses it for label text instead of card backgrounds; prefix byte `0x03` renders purple/magenta in-game. Full-card replicas keep their normal readable text; label colors are represented as softened, contrast-tested accents on dark-background card names, including result headings, lookup rows, autocomplete options, and relationship tables. Existing conflated or code-3-green color caches are invalidated by game-data cache version 16.
