...
| **Post-Duel Optimization Reliability** (DONE) | — | Keep the collection-change detector app-wide, preserve baselines across transient bridge resets and missing collection frames, keep optimization snapshots through hook remounts only while work is pending, fall back to selected-mod data if bridge game data is unavailable, abort hung optimizer runs, cancel pending work when a confirmed new active duel starts, and keep the results screen in the duel lifecycle until the game scene actually exits |
...
| **Campaign Reward Pool Evidence** (DONE) | [`docs/dropx15.md`](dropx15.md) | Capture rank counters and credited-card fit evidence; use the community Ghost/FMR x15 routine for compatible images, including Gold and the verified PAL French port |
