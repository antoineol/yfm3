...
| **Duel Best Play Path Selection** (DONE) | [`docs/steps/duel-best-play-path-selection.md`](steps/duel-best-play-path-selection.md) | Pick the displayed path for equivalent Best Plays by preserving the strongest remaining hand play before falling back to simpler and lower-ATK material paths |
