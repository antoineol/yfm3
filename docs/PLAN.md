...
| 6.5 (V2): Progress Reporting (DONE) | `docs/steps/phase-6.5-early-termination.md` | Progress reporting without convergence-stop; optimizer workers now run until their budget or explicit abort |
...
| **Deck Copy Limit Extraction** (DONE) | [`docs/deck-limit-findings.md`](deck-limit-findings.md) | Extract RP/Alpha dispatcher-table limits and vanilla inline Exodia one-copy limits, including PAL French `SLES_039.48`; cached game data is invalidated when extractor semantics change |
| **Post-Duel Optimization Reliability** (DONE) | — | Keep the collection-change detector app-wide, preserve collection/deck baselines across transient bridge resets, missing collection frames, transient empty deck definitions, and results-screen reconnects, start optimization as soon as reward cards appear even if the phase still looks active, keep post-duel work running through the next duel, hide stale post-duel UI once a new active hand is confirmed, fall back to selected-mod data if bridge game data is unavailable, abort hung optimizer runs, keep manual auto-sync collection totals tied to the last complete deck, and keep the results screen in the duel lifecycle until the game scene actually exits |
| **Duel Best Play Path Selection** (DONE) | [`docs/steps/duel-best-play-path-selection.md`](steps/duel-best-play-path-selection.md) | Pick the displayed path for equivalent Best Plays by preferring higher result DEF at equal ATK, fewer materials, the strongest remaining hand play using ATK then DEF, and lower-value material paths using ATK then DEF |
| **Duel Results HUD Hygiene** (DONE) | — | Hide active-duel helpers such as opponent-cheat warnings and cheat-view controls once the duel phase has ended; keep local opponent-cheat banners cleared on results and fresh duel remounts |
| **Campaign Reward Pool Evidence** (DONE) | [`docs/dropx15.md`](dropx15.md) | Capture rank counters and credited-card fit evidence; use the community Ghost/FMR x15 routine for compatible NTSC/Gold images, expose selectable PAL French card and starchip reward multipliers through the verified scratch relocation plus root save-update helper, restore the visible PAL reward from the credited scratch list, and execute-test the PAL helper arithmetic including PS1 load-delay behavior |
| **Bridge Distribution** | — | Portable Windows zip with in-app setup guide, update notification, and GitHub Actions release workflow |
| **Bridge Restart Resilience** (DONE) | [`docs/steps/bridge-restart-resilience.md`](steps/bridge-restart-resilience.md) | Preserve the last ready app state through short bridge live-reload reconnects, retry the bridge WebSocket quickly, ignore bridge-managed ISO backup folders during active-disc discovery, replay/request game-data results so clients recover naturally, and avoid whole-BIN reads during cache-miss game-data extraction |
...
| **Agent Notes Hygiene** (DONE) | [`docs/steps/agents-confusion-points-cleanup.md`](steps/agents-confusion-points-cleanup.md) | Keep `AGENTS.md` confusion points limited to active pitfalls; moved non-confusion work to `docs/TODO.md` |
...
| **Duel Cursor Target** (CURRENT) | [`docs/steps/duel-cursor-target.md`](steps/duel-cursor-target.md) | Show focused cards and target-selection battle prediction in the duel HUD without revealing opponent hidden cards unless cheat mode is enabled; PAL now maps target-card id plus empty-slot clearing, and cheat view resets stale Opponent state when a player phase starts |
| **PAL Bridge Feature Parity** (CURRENT) | [`docs/steps/pal-bridge-feature-parity.md`](steps/pal-bridge-feature-parity.md) | Map and verify PAL French bridge memory fields for cursor focus, rank counters, post-duel lifecycle, opponent focus, hand slots, free-duel unlocks, and localized equip bonuses; PAL terrain reads from `0x09C6F9`, PAL cards-left now uses the live deal counter during active duels and the result rank cards-used byte on results, PAL field cursor target reads from `0x09C6B8`, PAL field-card focus reads non-zero at `0x09C6D1`, and PAL card metadata keeps canonical type/guardian-star/attribute enums while names/descriptions/type labels stay localized |
