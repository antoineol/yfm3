...
- The community x15 drop recipe is Ghost's `Drop More Cards` approach: patch the root executable and `DATA/WA_MRG.MRG`, or the matching raw loop anchors. In `WA_MRG.MRG`, the full blob starts at `0xb4c400 + i*0x75800` for `i=1..7`; code starts 0x40 bytes later. Do not move this back to `0xb4bf00`: that was a misread offset and overwrote unrelated Gold WA data, causing duel-start crashes.
- Gold `SLUS_000.04` uses the same community x15 shape as vanilla/FMR, but has a local continuation instruction at `SLUS:0x1247c`; preserve it by writing only the two hook words at `0x1246c`. Failed custom attempts: buffered/visible-hook variants using `0x80021f24` crashed entering the result screen; stack/shared-state and no-stack expansion candidates in zeroed `SLUS:0x19b440` crashed entering the result screen; inline selector capture plus award-time extra picker calls did not crash but generated impossible low-card rewards such as `#1/#2/#3`. Ghidra C decompilation shows Gold still uses `FUN_80021810(selector)` to pick and `FUN_80021894(card)` to credit, with displayed reward picked during result init and credited later at `0x80021f10`. Do not add any patch that calls `FUN_80021810` at the later award site.
