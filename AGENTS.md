...
- Gold x15 currently uses the verified `buffered-picker-x15` root-executable-only patch. The extra-roll loop must reload the captured rank selector before every picker call; branching back to the picker call itself can use the previous card ID as the selector and produce impossible rewards like Blue-eyes from Meadow Mage.
