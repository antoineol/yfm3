...
- Vanilla deck-copy limits are not all "3": Exodia cards `17..21` are hard-capped at one copy by inline deck-edit code (`cardId - 17 < 5` plus `deckCount < 1`), even though vanilla has no RP/Alpha dispatcher table. PAL French `SLES_039.48` shows the same inline rule; do not treat `deckLimits: null` as correct for vanilla Exodia.
...
- PAL French x50/x150 keeps the shared `0x28590` display hook vanilla, so the bridge makes the result-state card restore from Ghost scratch slot 1 (`lhu v0,0x22(sp)` at blob offset `0xf4`) before returning to vanilla result setup. This keeps the single visible reward card inside the actual credited card list without hooking PAL script/text rendering. Older x50/x150 blobs that restore `lw v0,0(sp)` at that offset can show a vanilla pick that was not granted; the patch inspector recognizes them as refreshable but not enabled.
- Starchips are a separate reward-save arithmetic patch, not part of Ghost's card picker loop. Preserve the result-screen `rankStarchips` field and patch only the save update from `save += rankStarchips` to `save += rankStarchips * multiplier`. The byte anchor is `3a004390 e005828c 00000000 21104300 e00582ac`; the inline x15 form is `3a004390 e005828c 00290300 2318a300 21104300 e00582ac`. Verified runtime x15 results: PAL FR S-rank `22 -> 97`, NTSC S-rank `5 -> 80`, both with 15 cards. PAL French x50/x150 cannot fit inline; the bridge hooks the three-word `nop/addu/sw` tail to a helper at `SLES:0x19b700` / `0x801aaf00`, reloads the save starchip value inside the helper, includes a MIPS load-delay `nop`, and returns to the same cap-check continuation as the verified inline x15 patch. Without that `nop`, x150 with rank `5` writes `1490` from `148*5 + 150*5` instead of adding to the saved total. Because PAL reward flow can reload the Ghost area from `WA_MRG.MRG`, write this helper to the root executable and every PAL WA copy at `0xe25400 + i*0x78000 + 0x300`; root-only helpers crash after the reward flow reloads.
...
- PAL French live terrain is `0x09c6f9`, confirmed from the `SLES_039.48` field-bonus routine (`0x8009c6f9`). Do not derive it from the NTSC terrain relative offset, and do not leave PAL terrain unmapped: field display, best-play priority, battle prediction, and field-targeted optimization all depend on this byte.
...
- Post-duel reward snapshots can briefly carry an empty/all-zero `deckDefinition` while collection counts already include won cards. Do not consume the collection change with that invalid deck; use the last valid active/saved deck or wait for a complete deck, otherwise `currentDeckScore` becomes `null` and real swaps can be mislabeled as "no improvement".
- Bridge collection totals are trunk plus deck definition. If a poll reports an empty/all-zero `deckDefinition`, preserving the last complete deck is required for manual optimization too; otherwise deck cards disappear from the auto-sync collection and obvious owned upgrades can be missed.
- Manual and post-duel optimizations should not convergence-stop worker search before the allocated budget. A progress bar stopping around half time is suspicious: workers should finish naturally or be aborted by an explicit cancellation/timeout.
...
- Game-data cache validation must tolerate sub-millisecond `mtimeMs` differences. The same NTFS disc image has been observed with a fractional cached mtime and an integer Windows Bun stat mtime, which caused false cache misses and full 16s startup extraction.
- Bridge game-data extraction must not read the whole BIN just to extract ISO9660 files. Opened PAL BINs are ~548 MB, and Windows cold reads can make duplicate full-image reads look like a 20s+ bridge startup regression; use fd-based ISO file reads and streaming scans for raw-BIN tables.
- DuckStation process detection must not rely on `tasklist /FI "IMAGENAME eq duckstation*"`. On current Windows/DuckStation builds, the process can be named `duckstation-qt-x64-ReleaseLTCG.exe`, and that wildcard filter may return no rows. Enumerate tasklist rows and match image names starting with `duckstation` instead.
- Closing DuckStation's running game for ISO/memcard writes should use UI Automation to invoke the stable Qt action id `actionCloseGameWithoutSaving`. Do not rely on localized menu text or accelerators like `W`; the French UI translated this item and left the menu open. Menu navigation is only a fallback.
