...
describe("parseCardsGrid", () => {
  it("parses a valid monster row", () => {
    const cards = parseCardsGrid(
      cardsGrid(["1", "Dragon", "1200", "900", "Dragon", "", "", "red"]),
    );
  ...
  it("normalizes whitespace in name and kinds", () => {
    const cards = parseCardsGrid(
      cardsGrid(["1", "  Blue  Eyes  ", "3000", "2500", "  Dragon  ", "", "", ""]),
    );
  ...
  it("throws on invalid id (non-number)", () => {
    expect(() =>
      parseCardsGrid(cardsGrid(["abc", "Dragon", "1000", "800", "", "", "", ""])),
    ).toThrow("invalid id");
  });
...
describe("parseFusionsGrid", () => {
  it("parses a valid fusion row", () => {
    const fusions = parseFusionsGrid(
      fusionsGrid(["Dragon", "Eagle", "Sky Dragon", "2100", "1500"]),
    );
  ...
  it("skips blank rows", () => {
    const fusions = parseFusionsGrid(
      fusionsGrid(["", "", "", "", ""], ["A", "B", "C", "100", "200"]),
    );
    expect(fusions).toHaveLength(1);
  });

  it("throws on missing required field", () => {
    expect(() => parseFusionsGrid(fusionsGrid(["", "B", "C", "100", "200"]))).toThrow(
      "missing materialA",
    );
    expect(() => parseFusionsGrid(fusionsGrid(["A", "B", "C", "abc", "200"]))).toThrow(
      "invalid resultAttack",
    );
  });
