...
export function Dialog({ open, onClose, title, popupClassName, children }: DialogProps) {
  return (
    <BaseDialog.Root onOpenChange={(v) => !v && onClose()} open={open}>
      <BaseDialog.Portal>
        ...
        <BaseDialog.Popup
          className={
            popupClassName ??
            "fixed top-1/2 left-1/2 z-50 -translate-x-1/2 -translate-y-1/2 bg-bg-panel border border-border-accent rounded-xl p-5 shadow-overlay max-w-md w-full focus:outline-none"
          }
        >
