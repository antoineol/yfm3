...
describe("findBestDeckSwapSuggestion", () => {
  it("returns the best improving swap", () => {
    expect(
      findBestDeckSwapSuggestion({
        addedCardId: 5,
        config: { useEquipment: true, megamorphId: 657, deckSize: 5, fusionDepth: 3, terrain: 0 },
        deck: [1, 1, 2, 3, 4],
      }),
  ...
  it("returns null when the deck is not full", () => {
    expect(
      findBestDeckSwapSuggestion({
        addedCardId: 5,
        config: { useEquipment: true, megamorphId: 657, deckSize: 3, fusionDepth: 3, terrain: 0 },
        deck: [1, 1],
      }),
    ).toBeNull();
  });
  ...
  it("exact-scores the shortlisted candidates before picking the suggestion", () => {
    ...
    expect(
      findBestDeckSwapSuggestion({
        addedCardId: 5,
        config: { useEquipment: true, megamorphId: 657, deckSize: 6, fusionDepth: 3, terrain: 0 },
        deck: [1, 7, 2, 3, 4, 6],
      }),
  ...
  it("returns null when no swap strictly improves the score", () => {
    ...
    expect(
      findBestDeckSwapSuggestion({
        addedCardId: 5,
        config: { useEquipment: true, megamorphId: 657, deckSize: 2, fusionDepth: 3, terrain: 0 },
        deck: [1, 2],
      }),
    ).toBeNull();
  });
