...
describe("optimizeDeck", () => {
  ...
  it("throws on collection with fewer cards than scoringSlots", () => {
  ...
  it("throws on invalid scoringSlots", () => {
    expect(() => optimizeDeck(allCardsCollection(), { scoringSlots: 3 })).toThrow(
      /between 5 and 40/,
    );
    expect(() => optimizeDeck(allCardsCollection(), { scoringSlots: 50 })).toThrow(
      /between 5 and 40/,
    );
  });

  it("optimizes a 20-card deck", () => {
    const result = optimizeDeck(allCardsCollection(), { timeLimit: 8_000, scoringSlots: 20 });
